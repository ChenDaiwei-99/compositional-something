...
choose_repair_profile() {
  ...
  if [[ -f "${err_file}" ]] && rg -qi 'DUE TO TIME LIMIT|TIME LIMIT' "${err_file}"; then
    extra_args="--num-rounds 4 --batch-size 96 --eval-batch-size 192"
    use_fallback_profile=1
  ...
  if [[ ${use_fallback_profile} -eq 0 ]]; then
    extra_args="--batch-size 96 --eval-batch-size 192"
    use_fallback_profile=1
...
choose_quality_profile() {
  ...
  extra_args="--initial-train-per-digit 3000 --validation-per-digit 160 --initial-bootstrapping-epochs 12 --num-epochs 5 --growth-warmup-epochs 8 --learning-rate 2e-4 --batch-size 96 --eval-batch-size 192 --max-total-rounds 140 --saturation-patience 4 --saturation-delta 0.001 --bootstrap-new-examples-per-digit 500 --min-rounds-per-frontier-before-growth 4"
  use_fallback_profile=1
